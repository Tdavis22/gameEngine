...
void AnimatedSprite::addAnimation(int frames, int x, int y, std::string name, int width, int height, Vector2 offset) {
	std::vector<SDL_Rect> rectangles; /*rectangles of animation*/
...
void AnimatedSprite::update(int elapsedTime){
    ...
    if (this->_timeElapsed > this->_timeToUpdate) {
		...
		/* loop to next frame. Last frame loops to first*/
...
void AnimatedSprite::draw(Graphics &graphics, int x, int y) {
	if (this->_visable) {
		...
		SDL_Rect sourceRect = this->_animations[this->_currentAnimation /*vector in map*/][this->_frameIndex/*rect in vector*/];
...
/* needs to be implemented in specific character*/
