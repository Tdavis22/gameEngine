...
AnimatedTile::AnimatedTile(std::vector<Vector2> tilesetPostions, int duration,
	SDL_Texture * tileset, Vector2 size, Vector2 position) :
	Tile(tileset, size, tilesetPostions.at(0), position),
	_tilesetPositions(tilesetPostions),
	_duration(duration),
	_tileToDraw(0) 
{}
...
void AnimatedTile::draw(Graphics &graphics) {
	...
	SDL_Rect sourceRect = { this->_tilesetPositions.at(this->_tileToDraw).x,
